...
export const RippleButton = ({
  ...
  rippleDuration = 600,
  hoverBaseColor = '#445D48',
  hoverRippleColor: customHoverRippleColor,
  hoverBorderEffectColor = '#D6CC9977',
  ...
  const determinedJsRippleColor = useMemo(() => {
    ...
    return 'var(--button-ripple-color, rgba(214, 204, 153, 0.3))';
    ...
    if (variant === 'hover') {
      ...
      return `
        ...
        .hoverborder-variant-visual-ripple {
          ...
          transition: width ${hoverEffectDuration} ease-out, height ${hoverEffectDuration} ease-out, top 99999s linear, left 99999s linear;
        }
  ...
  if (variant === 'hover') {
    const hoverButtonFinalClassName = [
      "relative", "rounded-2xl", "text-base", "font-extrabold", "px-8", "py-4",
    ...
    return (
      <>
        ...
        <button className={hoverButtonFinalClassName} onClick={handleButtonClick} disabled={disabled} {...props}>
  ...
  if (variant === 'hoverborder') {
    const hoverBorderButtonFinalClassName = [
      "relative", "rounded-2xl", "overflow-hidden", "text-base", "font-extrabold", "px-8", "py-4",
  ...
  if (variant === 'ghost') {
    const ghostButtonFinalClassName = [
      ...
      "px-8", "py-4", "rounded-2xl", "text-base", "font-extrabold",
      disabled ? "opacity-50 cursor-not-allowed pointer-events-none" : "",
      className,
    ].filter(Boolean).join(" ");
    return (
      <>
        <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
        <button className={ghostButtonFinalClassName} onClick={handleButtonClick} disabled={disabled} {...props}>
          <span className="relative z-10 pointer-events-none flex items-center justify-center gap-2">{children}</span>
          {jsRippleElements}
        </button>
      </>
  ...
  // Default variant
  const baseClasses = "relative border-none overflow-hidden isolate transition-all duration-200 cursor-pointer px-8 py-4 bg-[#445D48] hover:opacity-90 text-[#FDE5D4] font-extrabold rounded-2xl";
  const disabledClasses = disabled ? "opacity-50 cursor-not-allowed" : "";
  const buttonClasses = `${baseClasses} ${disabledClasses} ${className}`;
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
      <button className={buttonClasses} onClick={handleButtonClick} disabled={disabled} {...props}>
        <span className="relative z-[1] pointer-events-none flex items-center justify-center gap-2">{children}</span>
        {jsRippleElements}
      </button>
    </>
  );
